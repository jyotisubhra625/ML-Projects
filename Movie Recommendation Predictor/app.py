...
def load_movies():
    ...
    if movies_df is None or movies_df.empty:
        st.error('Movie data file not found.')
        return pd.DataFrame()
...
def recommend(movie_title):
    ...
    top_candidates = scored_rows[:RECOMMENDATION_LIMIT]
        ...
        recommended.append(
            {
                ...
                'score': int(min(100, max(1, score * 100))),
...
st.info('The app is running in lightweight mode to avoid loading the full 1M-row matrix and stay responsive.')
movies = load_movies()
...
selected_poster = fetch_poster(selected_movie_id, get_tmdb_api_key(), selected_row.get('poster_path'))
